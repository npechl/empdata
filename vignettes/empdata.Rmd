...
`empdata` is a collection of metagenomic data originated from the [Earth Microbiome Project](https://www.earthmicrobiome.org/) (EMP). The package contains a subset of 975 sample with 150bp-length sequences. 

Full documentation regarding the EMP dataset can be found on the related article *[A communal catalogue reveals Earth’s multiscale microbial diversity](https://doi.org/10.1038/nature24621)*.

## Sample collection

Samples were gathered for microbiome analysis. 
Each sample sample was collected fresh and, where possible, immediately frozen in liquid nitrogen and stored at –80 °C. 
...
DNA extraction and 16S rRNA amplicon sequencing was done based on the [EMP standard protocols](http://www.earthmicrobiome.org/protocols-and-standards/16s).

## Tag sequence and subsets

The overall EMP dataset is provided throught the [FTP site](ftp://ftp.microbio.me/emp/release1/) (release1) and it contains a total of 97 studies and 27,742 samples.
...
Additionaly, samples where randomly picked in order to give equal (as possible) representation across environments (EMPO level 3) and across studies within those environments. 
...
The metadata file was additionaly filtered in order to contain the sample from the downloaded BIOM file. 
